...
def test_callback_invalid_code(client, mocker):
    ...
    mocker.patch("services.spotify_service.create_spotify_oauth", return_value=mock_spotify_oauth)

    response = client.get("/callback?code=invalid_code")

    # Should redirect to a fallback or error page
    assert response.status_code in (200, 302, 500)
    assert b"Authorization failed" in response.data or response.status_code == 302
